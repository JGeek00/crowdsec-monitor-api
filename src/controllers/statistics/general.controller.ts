...
export async function getStatistics(
  req: Request<object, Res, object, GetStatisticsQueryParams>,
  res: Response<Res>,
): Promise<void> {
  ...
  try {
    ...
    const whereClauseAlerts = sinceDate ? { [AlertsTable.col.crowdsecCreatedAt]: { [Op.gte]: sinceDate } } : {};

    const whereClauseDecisions = sinceDate ? { [DecisionsTable.col.crowdsecCreatedAt]: { [Op.gte]: sinceDate } } : {};
    ...
    const alertsByDate = (await AlertsTable.findAll({
      ...
      order: [
        [
          AlertsTable.sequelize!.fn('DATE', AlertsTable.sequelize!.col(AlertsTable.col.crowdsecCreatedAt)),
          DB_SORTING.ASC,
        ],
      ],
      raw: true,
    ...
    const decisionsByDate = (await DecisionsTable.findAll({
      attributes: [
        [
          DecisionsTable.sequelize!.fn('DATE', DecisionsTable.sequelize!.col(DecisionsTable.col.crowdsecCreatedAt)),
          'date',
        ],
    ...
    const scenariosData = await AlertsTable.findAll({
      attributes: ['scenario', [AlertsTable.sequelize!.fn('COUNT', AlertsTable.sequelize!.col('id')), 'count']],
      where: whereClauseAlerts,
      group: ['scenario'],
    ...
    res
      .status(500)
      .json(errorResponse('Error fetching statistics', error instanceof Error ? error.message : 'Unknown error'));
  } finally {
    cleanup();
  }
}
